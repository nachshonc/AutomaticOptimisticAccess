...
# how many tymes to repeat each test
num_runs=10
if [ $# -ge 2 ];
    then num_runs=$2
fi

num_initial_keys=10000
if [ $# -ge 3 ];
...
if [ ! -f $test_file_NoRecl ]; then
    echo "Test file $test_file_NoReclPalloc does not exist. Exiting"
    exit
fi
...
#                $test_file_NoReclMalloc $i $args >> $log_file_NoReclMalloc 2>&1        done
        done
        printf "$i "
    done
